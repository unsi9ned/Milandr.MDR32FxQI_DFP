...
uint32_t LEDS_Pin[6] = {
        PORT_Pin_5, PORT_Pin_8, PORT_Pin_9, PORT_Pin_10, PORT_Pin_11, PORT_Pin_15
};
...
int main(void)
{
    ...
    */
    ...
    /* Enables the HSI clock on PORTC and PORTD */
    ...
    /* In this infinite loop, joystick are iteratively scanned and
       ...
       joystick input pin numbers exactly match LED output pins, i.e:
       SEL    => VD8
       UP     => VD7
       DOWN   => VD11
       LEFT   => VD7
    ...
    while(1)
    {
        ...
        for(i = 0; i < 6; i++)
        {
            ...
            {
                PORT_SetBits(MDR_PORTD, 1 << (LED0_Pos + i));
            }
            else
            {
                PORT_ResetBits(MDR_PORTD, 1 << (LED0_Pos + i));
            }
        }
    }
}
